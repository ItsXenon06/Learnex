...
/* ─── CSS ─────────────────────────────────────────────────────────────── */
const css = `
/* Messages takes full height inside Layout's center column */
.msg-wrap {
  display: flex;
...
/* Unread indicator — left bar glow */
.cv-row.has-unread::before {
  content: '';
  position: absolute;
...
/* Mini avatar — only shown on last message in a group from that sender */
.msg-mini-av {
...
.msg-mini-av.ghost { visibility: hidden; } /* keeps spacing for grouped messages */
...
/* Bubble shape: last in group gets the "tail" corner */
...
.msg-status { color: var(--t4); font-size: 9px; }

/* Reply quote inside bubble */
.reply-quote {
  background: rgba(255,255,255,.08);
  border-left: 2px solid rgba(232,25,44,.5);
  border-radius: 6px;
  padding: 5px 10px;
  margin-bottom: 6px;
  font-size: 11px;
...
.send-btn:hover:not(:disabled) { transform: scale(1.08); box-shadow: 0 4px 16px var(--red-glow); }
...
.btn-fire-sm {
  ...
  font-family: var(--fb); letter-spacing: .8px; text-transform: uppercase;
...
.modal-head {
  ...
  padding: 16px 20px;
  border-bottom: 1px solid var(--b1);
  background: linear-gradient(90deg, rgba(232,25,44,.06) 0%, transparent 70%);
}
...
.mfield input {
  ...
  font-size: 14px; font-family: var(--fb); outline: none;
  transition: border-color .2s, box-shadow .2s;
}
...
/* Mobile: hide conv list, show chat only */
@media (max-width: 700px) {
  .cl { display: none; }
  .cl.show { display: flex; position: fixed; top: var(--tb); left: 0; right: 0; bottom: 0; z-index: 200; background: var(--bg); width: 100%; }
}
`;
...
export default function MessagesPage() {
  ...
  const sendMsg = async () => {
    ...
    try {
      ...
      setConvs(prev => prev.map(c =>
        c.id === activeId ? { ...c, lastMessage: saved } : c
      ));
    } catch {
  ...
  const filtered = convs.filter(c => {
    ...
    const name = c.otherUserDisplayName || c.otherUserEmail || c.name || '';
    return name.toLowerCase().includes(searchQ.toLowerCase());
  });

  const otherName = activeConv?.otherUserDisplayName || activeConv?.otherUserEmail || activeConv?.name || 'Conversation';
  const otherIni  = getInitials(activeConv?.otherUserDisplayName, activeConv?.otherUserEmail);
          ...
          <div className="cl">
            <div className="cl-head">
              ...
              <button
                ...
                onClick={() => { setDmOpen(true); setDmErr(''); setDmEmail(''); }}
              >
                ✎
              </button>
            </div>
            ...
            <div className="cl-body">
              {convsLoading ? <ConvSkeleton /> :
                filtered.length === 0 ? (
                  ...
                  const name    = conv.otherUserDisplayName || conv.otherUserEmail || conv.name || 'Unknown';
                  const ini     = getInitials(conv.otherUserDisplayName, conv.otherUserEmail);
                  ...
                  return (
                    <div
                      key={conv.id}
                      className={`cv-row ${activeId === conv.id ? 'active' : ''} ${unread && activeId !== conv.id ? 'has-unread' : ''}`}
          ...
          <div className="chat-panel">
            {!activeId ? (
              <div className="chat-empty">
                ...
                <p className="chat-empty-s">Pick a conversation or start a new one with a classmate.</p>
                <button
              ...
              <>
                    ...
                    messages.length === 0 ? (
                      ...
                      groups.forEach((group, gi) => {
                        ...
                        const msgElements = group.messages.map((msg, mi) => {
                          ...
                          const isFirst = mi === 0;
                          // Grouped = not the last bubble → no tail
                ...
                {draft && (
                  <div className="compose-hint">Shift+Enter for new line · Enter to send</div>
                )}
              </>
            )}
          </div>
